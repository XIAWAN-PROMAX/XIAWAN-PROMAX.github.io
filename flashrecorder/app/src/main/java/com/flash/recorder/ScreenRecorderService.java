...
public class ScreenRecorderService extends Service {
    ...
    static final int STATE_TRANSITION = 2;
    static final int STATE_SAVING = 3;
    private static final int PRE_BUFFER_US = 30_000_000;
    ...
    private void audioLoop() {
        final int inFrameSamples = 1024;
        final int inFrameBytes = inFrameSamples * 2;
    ...
    private synchronized void handleFrame(boolean isVideo, ByteBuffer buf, MediaCodec.BufferInfo src) {
        if (state == STATE_BUFFERING) {
            ...
            if (track >= 0) {
                ...
                try { muxer.writeSampleData(track, buf, adj); } catch (Exception e) {}
            }
        }
    }
    ...
    public synchronized void requestStartSaving() {
        ...
        try {
            currentOutputFile = new File(VideoStore.getOutputDir(this),
            ...
            for (Frame f : videoBuffer.drainAll()) writeFrame(videoTrack, f, savePtsOffsetUs);
            if (audioTrack >= 0) {
                for (Frame f : audioBuffer.drainAll()) writeFrame(audioTrack, f, savePtsOffsetUs);
            }
    ...
    public synchronized void requestStopSaving() {
        ...
        try { if (muxerStarted) muxer.stop(); muxer.release(); } catch (Exception e) {}
        muxer = null; muxerStarted = false;
        savePtsOffsetUs = 0;
    ...
    private synchronized void shutdown() {
        running = false;
        try { if (videoThread != null) videoThread.interrupt(); } catch (Exception ignored) {}
